...
if __name__ == "__main__":
    ...
    # Record data for mental activity 2
    winsound.Beep(400,1000) # Beep sound
    eeg_data2, timestamps2 = inlet.pull_chunk(
            timeout=training_length+1, max_samples=fs * training_length)
    ...
    eeg_epochs1 = BCIw.epoch(eeg_data1, epoch_length * fs,
                             overlap_length * fs)
    eeg_epochs2 = BCIw.epoch(eeg_data2, epoch_length * fs,
                             overlap_length * fs)
    ...
    [classifier2, mu_ft2, std_ft2] = BCIw.train_classifier(
            feat_matrix0, feat_matrix2, 'SVM')
    ...
    plotter_decision2 = BCIw.DataPlotter(30, ['Decision'])
    ...
    try:
        while True:
            ...
            y_hat2 = BCIw.test_classifier(classifier2,
                                         feat_vector.reshape(1, -1), mu_ft2,
                                         std_ft2)
            print([y_hat,y_hat2])
            ...
            decision_buffer2, _ = BCIw.update_buffer(decision_buffer,
                                                    np.reshape(y_hat2, (-1, 1)))

            """ 3.3 VISUALIZE THE DECISIONS """
            plotter_decision.update_plot(decision_buffer)
            plotter_decision2.update_plot(decision_buffer2)
            plt.pause(0.00001)
